...
TEST_CASE_METHOD(HelixTestFixture, "shipped filaments.json is well-formed", "[filament_data]") {
    ...
    for (const auto* p : all) {
        ...
        CHECK(filament::find_material(p->type).has_value());  // every type resolves
...
Expected: PASS. If a `find_material` assertion fails, extend `TYPE_MAP` in the importer (Task 1) so that Orca type maps to a known type, then `make regen-filaments`.

- [ ] **Step 3: Add the Android-mirror identity check to CI lint**

Append to `tests/shell/test_code_lint.bats`:
```bash
@test "filaments.json android mirror is byte-identical" {
