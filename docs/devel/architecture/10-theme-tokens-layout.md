...
`PanelWidgetConfig::build_default_grid()` (`src/system/panel_widget_config.cpp:536`) reads the file through `helix::find_readable()` at runtime — a user's config-dir copy wins over the shipped asset, so layouts are editable on-device without a rebuild. Widgets not listed in any table are auto-placed after the anchored ones.
...
6. `src/ui/theme_manager.cpp:1085` — `theme_manager_resolve_px_tokens()`: the two-axis suffix choice, the `nav_width` special case, the required-triplet rule.
7. `src/ui/theme_manager.cpp:1490` — `theme_manager_register_semantic_colors()`: theme JSON palette → `card_bg`/`card_bg_light`/`card_bg_dark` consts.
8. `src/ui/theme_manager.cpp:2575` — `theme_manager_get_color()`: the `_light`/`_dark` probe order and the partial-variant error path.
9. `src/ui/theme_manager.cpp:1974` — `theme_manager_apply_theme()`: the whole live-switch sequence from swap maps to generation bump.
...
14. `src/system/panel_widget_config.cpp:536` — `build_default_grid()` loading those anchors through `variant_chain()`.
