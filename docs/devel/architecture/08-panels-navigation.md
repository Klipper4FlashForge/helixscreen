...
- **Component names must match filenames.** `get_xml_component_name()` / `component_name()` return the `ui_xml/` filename without `.xml` (`include/ui_panel_base.h:136`); a mismatch is the "panel created but empty" silent failure — the create succeeds, no component by that name exists. The six roots have a second name dependency: `PanelFactory::PANEL_NAMES` (`include/panel_factory.h:34`) must match the widget names in `ui_xml/app_layout.xml:18`-28 or boot fails to find the panels.
...
- **`push_overlay(root, false)` — the `hide_previous` variant — exists for split-view stacking.** Nothing in the tree uses it today; both overlays stay visible and both lifecycle maps see only the top. Do not reach for it without a reason this specific.
...
- `../ARCHITECTURE.md` — § Panel & Overlay Base Classes (the `on_deactivate()` vs `cleanup()` callout in full), § Navigation History Stack with code snippets, § Modal System (ui_dialog) overview.
...
5. `include/ui_panel_base.h:77` — `PanelBase`: dependency injection, two-phase init, observer RAII. Note `setup()` at `:123` and the component-name contract at `:136`.
