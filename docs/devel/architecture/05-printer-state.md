...
Registration order is load-bearing: `SubjectInitializer` initializes `NavigationManager` **after** `PrinterState` precisely so reverse-order deinit clears NavigationManager's observers on PrinterState subjects before those subjects die (the comment at `src/application/subject_initializer.cpp:250`-252).
...
1. `include/printer_state.h:185` — the `PrinterState` class doc, then jump to `:2252` and read the thirteen domain members: plain by-value composition, no pointers, no inheritance.
2. `src/printer/printer_state.cpp:196` — `init_subjects()`: the ordered domain fan-out, and the single `StaticSubjectRegistry::register_deinit("PrinterState", ...)` at the end. Then `:133` `deinit_subjects()` for the mirror image — guard invalidation, cache unregistration, token expiry, reverse fan-out.
...
8. `src/application/subject_initializer.cpp:473` — where the controller is constructed and registered as a `PanelWidgetManager` shared resource; scroll up to `:308` for the PrinterState init phase and its ordering comments.
