...
There is exactly one thread that may call `lv_*` anything: the main thread, which enters `Application::main_loop()` ([`src/application/application.cpp:3949`](../../../src/application/application.cpp#L3949)) and never leaves it until shutdown. Everything else is background:
