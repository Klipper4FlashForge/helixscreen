...
There is exactly one thread that may call `lv_*` anything: the main thread, which enters `Application::main_loop()` (`src/application/application.cpp:864`) and never leaves it until shutdown. Everything else is background:
