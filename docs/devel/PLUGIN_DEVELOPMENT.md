...
Subjects registered via `api->register_subject()` are tracked but not actually unregistered from LVGL's XML system during cleanup (LVGL 9.5 does not provide `lv_xml_unregister_subject()`). Re-loading a plugin with the same subject names works fine (registration is idempotent). Only affects development scenarios with frequent plugin reloads.
