...
**Total Hidden Tests:** 6

Hidden tests are excluded from normal runs using Catch2's `[.]` tag prefix. They exist for legitimate reasons (network dependencies, benchmarks, stress tests, destructive global-state cycles) and should be run manually when relevant.
