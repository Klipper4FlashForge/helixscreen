...
- **Provides standard error toasts** — on a failed RPC (or a heater that resolves empty), it emits the standard `NOTIFY_ERROR` toast and fires optional `on_success` / `on_error` hooks. `SendOptions{.toast = false}` suppresses **the controller's own** toast, for silent sends like AMS slot-preheat / cooldown — it does not suppress the failure. The flag is forwarded to `set_temperature()` as `caller_surfaces_errors`, so a send with `.toast = false` deliberately leaves Klipper's `!!` broadcast free to report the rejection through `GcodeErrorRouter` (see `RPC_ERROR_OWNERSHIP.md`). It holds no LVGL widgets or subjects — it uses the `NOTIFY_*` notification system, so the logic is unit-testable.
