...
- **Transport:** one `esp_websocket_client_handle_t`; config: `buffer_size=32768`, `task_stack=8192` (4096 default is too small once nlohmann is on the callback path), `reconnect_timeout_ms=10000`, `network_timeout_ms=10000`, `ping_interval_sec=10`, plain `ws://` only (reject `wss://` with an error log in v1).
...
- **Memory:** no allocation in the hot notify path beyond the json parse itself; reassembly buffer reused across messages (grow-only up to cap, `shrink_to_fit` on disconnect).
...
7. After 60s: log `I net_hil: PASS msgs=%u drops=%u heap_free=%u psram_free=%u` + heap-flat check (free-size counters ONLY — no heap walks, display is live).
...
- AMS vendor-backend HTTP polling decision (AD5X-IFS/QIDI poll `download_file` every 5s — open question from api research §5).
