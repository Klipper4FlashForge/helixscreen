...
**Printing from bypass is the other exception.** When bypass is engaged, filament goes straight to the extruder without passing through any slot, so there is no slot for the check to look at and every tool would read as unfed. Rather than warn about all of them, HelixScreen skips the slot checks for as long as bypass stays engaged — and tells you so: the file details show a **Bypass active** note, and if the material you set on the external spool doesn't match what the file was sliced for (for example, the file says ASA-GF but the spool holds plain ASA), you get one **Material Mismatch** warning naming the spool. Tap **Start Anyway** to print regardless. Disengage bypass and the slot checks come back on the next print — you don't have to restart anything.
