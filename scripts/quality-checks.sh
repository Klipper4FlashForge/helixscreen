...
if [ -f "scripts/check_namespace_compliance.py" ]; then
  ...
  # ratcheting, so the merge collects that slack too. 2216 -> 2240 is this
  ...
  # own baseline is 2216, so the 24 are a ratchet this fork owes, not slack it
  # took.
  if python3 scripts/check_namespace_compliance.py --max-allowed 2240 --summary >/tmp/namespace_check.out 2>&1; then
    section_time $SECTION_START
    echo ""
