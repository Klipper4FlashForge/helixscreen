...
| `check_update_queue_leaks.py` | Ratcheting gate on cross-test UpdateQueue leaks (#1166). Parses `[ISOLATION-LEAK]` reports from a test log and fails on any leaking test absent from `--baseline scripts/update_queue_leak_baseline.txt`. A SET of test names, not a count: the count is timing- and order-dependent (305/305/302 across identical unsharded runs; 286 across 8 shards), so a count ratchet would flap red. `--max-allowed N` exists but is the weaker mode. The `2>&1` is required — the listener reports on stderr, Catch2's summary on stdout, and the gate rejects a log missing either rather than reading silence as zero leaks. Runs unsharded in nightly (`test-unsharded-leaks`; 769s). Also `--list` / `--by-producer` / `--write-baseline` |
