...
TEST_CASE("is_active_print_state: only PRINTING and PAUSED are active",
          "[print][navigation]") {
...
TEST_CASE("Print start nav: inactive -> active edges navigate", "[print][navigation]") {
    SECTION("STANDBY -> PRINTING (normal print start)") {
        REQUIRE(print_start_nav_should_navigate(PrintJobState::STANDBY,
    ...
    SECTION("COMPLETE -> PRINTING (retry after finished print)") {
        REQUIRE(print_start_nav_should_navigate(PrintJobState::COMPLETE,
                                                PrintJobState::PRINTING));
    }
    SECTION("ERROR -> PAUSED (recovery after error)") {
        REQUIRE(print_start_nav_should_navigate(PrintJobState::ERROR,
                                                PrintJobState::PAUSED));
    }
    SECTION("CANCELLED -> PRINTING (new print after cancel)") {
        REQUIRE(print_start_nav_should_navigate(PrintJobState::CANCELLED,
                                                PrintJobState::PRINTING));
    }
}

TEST_CASE("Print start nav: active -> active transitions do NOT navigate",
          "[print][navigation]") {
    SECTION("PAUSED -> PRINTING (mid-session resume must not yank user back)") {
        REQUIRE_FALSE(print_start_nav_should_navigate(PrintJobState::PAUSED,
                                                      PrintJobState::PRINTING));
    }
    SECTION("PRINTING -> PAUSED (pause)") {
        REQUIRE_FALSE(print_start_nav_should_navigate(PrintJobState::PRINTING,
                                                      PrintJobState::PAUSED));
    }
...
TEST_CASE("Print start nav: transitions to inactive states do NOT navigate",
    ...
    SECTION("STANDBY -> COMPLETE") {
        REQUIRE_FALSE(print_start_nav_should_navigate(PrintJobState::STANDBY,
