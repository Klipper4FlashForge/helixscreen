...
constexpr int32_t kNavHeight = 112; // button_height_lg at the xlarge tier
constexpr int32_t kGap = 16;        // space_lg
} // namespace
...
TEST_CASE("Landscape overlays span the full height", "[theme][overlay-height][portrait]") {
    ...
    for (const auto& c : cases) {
        INFO(c.label);
        auto hgt = compute_overlay_heights(c.w, c.h, kNavHeight, kGap);
...
TEST_CASE("Portrait overlay no longer covers the navigation bar",
    ...
    const auto portrait = compute_overlay_heights(480, 800, kNavHeight, kGap);
    constexpr int32_t old_height = 800;

    CHECK(portrait.destination == 688);
    CHECK(old_height - portrait.destination == kNavHeight);
...
TEST_CASE("Height and width classification agree on orientation",
    ...
    for (const auto& c : portrait_cases) {
        INFO("size " << c.w << "x" << c.h);
        const auto wid = compute_overlay_widths(c.w, c.h, 54, kGap);
        const auto hgt = compute_overlay_heights(c.w, c.h, kNavHeight, kGap);
