...
// ============================================================================
// Per-tool spool_id → printer gcode sync (#1079)
//
// assign_spool() pushes the spool_id into the printer's gcode macros / saved
// variables so Fluidd/Mainsail and per-tool macro logic see the assignment.
// Each half is independently detection-gated so Klipper never throws:
//   - SET_GCODE_VARIABLE MACRO=T<n> VARIABLE=spool_id VALUE=<id>
//       only when `gcode_macro t<n>` declares variable_spool_id.
//   - SAVE_VARIABLE VARIABLE=t<n>__spool_id VALUE=<id>
//       only when [save_variables] is configured.
// ============================================================================

namespace {

enum class MacroMode {
    NONE,           // no gcode_macro t2 at all
    PRESENT_NO_VAR, // gcode_macro t2 exists but does NOT declare variable_spool_id
    DECLARES_VAR,   // gcode_macro t2 declares variable_spool_id
};

/// Wires the global PrinterState + a MoonrakerAPI over a MoonrakerClientMock so
/// ToolState::assign_spool() can emit gcode, and captures every emitted script
/// via the mock's gcode_script_history(). Builds a 3-tool toolchanger so tool
/// index 2 is valid.
struct SpoolSyncHarness {
    MoonrakerClientMock client{MoonrakerClientMock::PrinterType::VORON_24};
    MoonrakerAPI api;

    SpoolSyncHarness(MacroMode mode, bool has_save_vars) : api(client, get_printer_state()) {
        lv_init_safe();

        // Global PrinterState must have subjects and be READY so execute_gcode's
        // halted-gate lets the gcode through.
        get_printer_state().init_subjects(false);
        get_printer_state().set_klippy_state_sync(helix::KlippyState::READY);

        helix::PrinterDiscovery hw;
        nlohmann::json objects = nlohmann::json::array({"toolchanger", "tool T0", "tool T1",
                                                        "tool T2", "extruder", "extruder1",
                                                        "extruder2", "heater_bed", "gcode_move"});
        if (has_save_vars) {
            objects.push_back("save_variables");
        }
        hw.parse_objects(objects);

        nlohmann::json cfg = nlohmann::json::object();
        if (mode == MacroMode::PRESENT_NO_VAR) {
            cfg["gcode_macro t2"] = {{"gcode", "M117 T2"}};
        } else if (mode == MacroMode::DECLARES_VAR) {
            cfg["gcode_macro t2"] = {{"variable_spool_id", "0"}, {"gcode", "M117 T2"}};
        }
        if (has_save_vars) {
            cfg["save_variables"] = {{"filename", "~/printer_data/vars.cfg"}};
        }
        hw.parse_config_keys(cfg);
...
bool history_contains(const std::vector<std::string>& hist, const std::string& sub) {
...
TEST_CASE("ToolState: assign_spool emits SET_GCODE_VARIABLE when macro declares variable_spool_id",
          "[tool][tool-state][spool][spool-sync]") {
    SpoolSyncHarness h(MacroMode::DECLARES_VAR, /*has_save_vars=*/false);

    ToolState::instance().assign_spool(2, 55, "Red PLA", 750.0f, 1000.0f);

    REQUIRE(history_contains(h.client.gcode_script_history(),
                             "SET_GCODE_VARIABLE MACRO=T2 VARIABLE=spool_id VALUE=55"));
    // No [save_variables] configured → no SAVE_VARIABLE.
    REQUIRE_FALSE(history_contains(h.client.gcode_script_history(), "SAVE_VARIABLE"));
}

TEST_CASE("ToolState: assign_spool skips SET_GCODE_VARIABLE when macro lacks variable_spool_id",
          "[tool][tool-state][spool][spool-sync]") {
    SpoolSyncHarness h(MacroMode::PRESENT_NO_VAR, /*has_save_vars=*/false);

    ToolState::instance().assign_spool(2, 55, "Red PLA", 750.0f, 1000.0f);

    REQUIRE_FALSE(history_contains(h.client.gcode_script_history(), "SET_GCODE_VARIABLE"));
}

TEST_CASE("ToolState: assign_spool skips SET_GCODE_VARIABLE when no T<n> macro exists",
          "[tool][tool-state][spool][spool-sync]") {
    SpoolSyncHarness h(MacroMode::NONE, /*has_save_vars=*/false);
...
TEST_CASE("ToolState: assign_spool emits SAVE_VARIABLE when [save_variables] is configured",
          "[tool][tool-state][spool][spool-sync]") {
    SpoolSyncHarness h(MacroMode::NONE, /*has_save_vars=*/true);

    ToolState::instance().assign_spool(2, 55, "Red PLA", 750.0f, 1000.0f);

    REQUIRE(history_contains(h.client.gcode_script_history(),
                             "SAVE_VARIABLE VARIABLE=t2__spool_id VALUE=55"));
    // Macro absent → no SET_GCODE_VARIABLE.
    REQUIRE_FALSE(history_contains(h.client.gcode_script_history(), "SET_GCODE_VARIABLE"));
}

TEST_CASE("ToolState: assign_spool skips SAVE_VARIABLE without [save_variables]",
          "[tool][tool-state][spool][spool-sync]") {
    SpoolSyncHarness h(MacroMode::DECLARES_VAR, /*has_save_vars=*/false);
...
TEST_CASE("ToolState: assign_spool emits both when macro var and save_variables present; "
          "neither when both absent",
          "[tool][tool-state][spool][spool-sync]") {
    SECTION("both present → both emitted") {
        SpoolSyncHarness h(MacroMode::DECLARES_VAR, /*has_save_vars=*/true);

        ToolState::instance().assign_spool(2, 77, "Blue PETG", 200.0f, 500.0f);

        REQUIRE(history_contains(h.client.gcode_script_history(),
                                 "SET_GCODE_VARIABLE MACRO=T2 VARIABLE=spool_id VALUE=77"));
        REQUIRE(history_contains(h.client.gcode_script_history(),
                                 "SAVE_VARIABLE VARIABLE=t2__spool_id VALUE=77"));
    }

    SECTION("neither present → nothing emitted, no crash") {
        SpoolSyncHarness h(MacroMode::NONE, /*has_save_vars=*/false);

        ToolState::instance().assign_spool(2, 77, "Blue PETG", 200.0f, 500.0f);

        REQUIRE(h.client.gcode_script_history().empty());
    }
}

TEST_CASE("ToolState: assign_spool unassign (spool_id 0) still emits VALUE=0",
          "[tool][tool-state][spool][spool-sync]") {
    SpoolSyncHarness h(MacroMode::DECLARES_VAR, /*has_save_vars=*/true);

    // First assign a real spool.
    ToolState::instance().assign_spool(2, 55, "Red PLA", 750.0f, 1000.0f);
    h.client.clear_gcode_script_history();

    // Now unassign (spool_id → 0): downstream state must be cleared, so VALUE=0
    // is emitted for both halves.
    ToolState::instance().assign_spool(2, 0, "", -1.0f, -1.0f);

    REQUIRE(history_contains(h.client.gcode_script_history(),
                             "SET_GCODE_VARIABLE MACRO=T2 VARIABLE=spool_id VALUE=0"));
    REQUIRE(history_contains(h.client.gcode_script_history(),
                             "SAVE_VARIABLE VARIABLE=t2__spool_id VALUE=0"));
}

TEST_CASE("ToolState: assign_spool does not re-emit when spool_id is unchanged",
          "[tool][tool-state][spool][spool-sync]") {
    SpoolSyncHarness h(MacroMode::DECLARES_VAR, /*has_save_vars=*/true);

    ToolState::instance().assign_spool(2, 55, "Red PLA", 750.0f, 1000.0f);
    REQUIRE_FALSE(h.client.gcode_script_history().empty()); // first assign emitted
    h.client.clear_gcode_script_history();

    SECTION("identical re-assign is a full no-op (early return)") {
        ToolState::instance().assign_spool(2, 55, "Red PLA", 750.0f, 1000.0f);
        REQUIRE(h.client.gcode_script_history().empty());
    }

    SECTION("same spool_id but changed weight/name → no gcode re-emit") {
        ToolState::instance().assign_spool(2, 55, "Red PLA (renamed)", 500.0f, 1000.0f);
        REQUIRE(h.client.gcode_script_history().empty());
    }
}
