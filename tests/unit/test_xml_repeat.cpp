...
// label must get its own $i and a fresh #const resolution per iteration. If the
// shared-attrs bug is present the labels read the same value (e.g. "2","2","2")
// or empty, and the #const fails to re-resolve on later iterations.
...
TEST_CASE_METHOD(LVGLTestFixture, "repeat: $i + $param + #const yield independent per-iteration values", "[xml][repeat]") {
    ...
    lv_obj_t * v = (lv_obj_t*)lv_xml_create(lv_screen_active(), "t_rep_i", nullptr);
    REQUIRE(v != nullptr);
    lv_obj_t * root = lv_obj_find_by_name(v, "root");
    REQUIRE(root != nullptr);
    REQUIRE(lv_obj_get_child_count(root) == 3);
    // each label's text is its own index 0,1,2 — proves attrs are pristine per iteration
    for (uint32_t k = 0; k < 3; k++) {
        lv_obj_t * lbl = lv_obj_get_child(root, k);
        REQUIRE(lbl != nullptr);
        char expect[8]; snprintf(expect, sizeof expect, "%u", k);
        REQUIRE(std::string(lv_label_get_text(lbl)) == expect);
