...
void app_main(void) {
    ...
    lv_color_t* buf1 =
        heap_caps_aligned_alloc(16, buf_px * sizeof(lv_color16_t), MALLOC_CAP_SPIRAM);
    lv_color_t* buf2 =
        heap_caps_aligned_alloc(16, buf_px * sizeof(lv_color16_t), MALLOC_CAP_SPIRAM);
    assert(buf1 && buf2);
    ...
    lv_subject_init_string(&s_counter_subject, s_counter_buf, s_counter_prev, sizeof(s_counter_buf),
                           "init");
...
static void cjk_experiment(void) {
    ...
    int64_t t1 = esp_timer_get_time();
    ESP_LOGI(TAG, "[cjk:binfont] %s in %lldms | psram cost=%d internal cost=%d",
    ...
    if (binfont)
        lv_obj_set_style_text_font(l1, binfont, 0);
...
static void fps_benchmark(void) {
    ...
    int64_t t1 = esp_timer_get_time();
    ESP_LOGI(TAG, "[fps] %d full-screen refreshes in %lldms = %.1f fps sustained", N,
             (t1 - t0) / 1000, (double)N * 1000000.0 / (double)(t1 - t0));
}
...
static void* app_phase(void* arg) {
    ...
    while (true) {
        ...
        if (now - last_update > 1000000) { // 1Hz subject updates through the binding
            last_update = now;
            n++;
            char tmp[64]; // must not write subject's own buffer: change detection compares against
                          // it
