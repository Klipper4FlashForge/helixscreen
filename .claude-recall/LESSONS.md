...
- **Uses**: 14 | **Velocity**: 0.01318359375 | **Learned**: 2026-02-21 | **Last**: 2026-03-31 | **Category**: ui | **Type**: constraint
...
- **Uses**: 1 | **Velocity**: 0.25 | **Learned**: 2026-07-02 | **Last**: 2026-07-02 | **Category**: correction
...
### [L099] [-----|-----] Base lv_obj IS clickable by default; clickable="false" does NOT inherit to children
- **Uses**: 1 | **Velocity**: 0 | **Learned**: 2026-07-15 | **Last**: 2026-07-15 | **Category**: gotcha
> `lv_obj_constructor` sets `obj->flags = LV_OBJ_FLAG_CLICKABLE` (`lib/lvgl/src/core/lv_obj.c:584`), so EVERY plain `<lv_obj>` layered over a clickable ancestor silently swallows its clicks. `lv_image` (lv_image.c:703) and `lv_label` (lv_label.c:770) both `lv_obj_remove_flag(CLICKABLE)` — so images/text/`icon`/`text_*` pass clicks through. That asymmetry is the tell: "thumbnail works, text area dead" (#1101 print_file_card metadata overlay). WIDELY MISBELIEVED FALSE — two Explore agents independently asserted "lv_obj is not clickable by default, only lv_button is" and would have closed #1101 as cannot-reproduce. Read the vendored constructor, don't trust recall (incl. your own). Second trap: `lv_indev_search_obj` (`lv_indev.c:618`) recurses into children on GEOMETRY ALONE (`lv_area_is_point_on(&obj_coords,...)`) — a parent marked `clickable="false"` does NOT stop its children being hit-tested, and the deepest hit-testing child wins. So fix EVERY offending descendant, not just the top overlay: #1101 needed 4 (`metadata_overlay`, `metadata_row`, 2 inner wrappers); the reporter's suggested overlay-only fix left the time/filament strip dead. Convention: `clickable="false" event_bubble="true"` (425 lines / 79 files). CHEAP EMPIRICAL CHECK, no indev needed: `lv_point_t p{x,y}; lv_indev_search_obj(test_screen(), &p)` in an `XMLTestFixture` test asserts which object a tap actually resolves to — same fn the real input path calls (`lv_indev.c:1663`). See `tests/unit/test_print_file_card_hittest.cpp`.
