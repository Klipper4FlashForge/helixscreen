...
**Klippy-volatile subjects (MANDATORY):** Moonraker sends DELTA updates — a subject fed by a delta-only field (e.g. `idle_timeout.state`) keeps its last value across a Klipper restart, since the field is simply absent until it next changes. If a stale value like that gates behavior, declare it with `INIT_SUBJECT_INT_VOLATILE(name, default, subjects_, volatile_, register_xml)` (`include/state/subject_macros.h`) instead of `INIT_SUBJECT_INT`, so a Klippy state transition resets it. NOT for continuously-streamed state (temps, positions) — those self-heal within a tick. See `docs/devel/ARCHITECTURE.md` § "Klippy-Volatile Subjects" (#1129).
