...
void MoonrakerClientMock::populate_capabilities() {
    ...
    case PrinterType::FLASHFORGE_CREATOR5:
        ...
        // Tool-offset state. The query/subscribe handlers already synthesize
        // ff_tool_offset and ff_tool <n> status (moonraker_client_mock_objects.cpp),
        // but nothing advertised them, so the tool-offset calibration screen was
        // unreachable under --test.
        mock_objects.push_back("ff_tool_offset");
        for (int i = 0; i < 4; ++i) {
            mock_objects.push_back("ff_tool " + std::to_string(i));
        }
...
int MoonrakerClientMock::gcode_script(const std::string& raw_gcode) {
    ...
    if (gcode.find("SET_HEATER_TEMPERATURE") != std::string::npos) {
        ...
        if (heater_name == "extruder") {
            ...
            return 1;
        } else if (const std::string bare = chamber_heater_bare_name();
                   !bare.empty() && heater_name == bare) {
            // Chamber heater, matched by the BARE object name the resolved
            // chamber heater uses — "chamber" for keyword heaters,
            // "dragonbreath" for backend-named ones (a hard-coded "chamber"
            // comparison silently ignores those).
