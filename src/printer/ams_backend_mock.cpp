...
void AmsBackendMock::set_afc_mode(bool enabled) {
    ...
    if (enabled) {
        ...
        for (int i = 0; i < lane_count; ++i) {
            ...
            // ams_backend_afc.cpp looks for spool_vendor/vendor_name/vendor/brand,
            // but upstream AFC #808 has not shipped (#833 is the PR), so the
            // payload never carries any of them and a real lane's brand stays
            // empty unless the user sets an override. The vendor in sample_data is what the
            // Spoolman identity cache supplies for the linked lanes, not what firmware reports --
            // seeding it onto the slot made mock mode render a brand real hardware never produces,
            // which is what hid #1264.
