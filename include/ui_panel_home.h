...
class HomePanel : public PanelBase {
  public:
    ...
    void finalize_setup();
    /// setup() is deliberately minimal, so the base rebuild() leaves the fresh
    /// widget tree empty and the recycled panel widgets still bound to the old
    /// one. Re-run the finalize pass on the new tree.
